...
def best_fit(items: list[float], assignment: list[int], free_space: list[float]):
    ...
    for i in range(n):
        ...
        if bin_found is not None:  # place item in bin and update the tree
            ...
            bin_key_d = dec.Decimal('{:.2f}'.format(bin_key))
            new_key_d = bin_key_d - item_d
            assignment[i] = bin_found.val
            free_space[bin_found.val] = float(dec.Decimal('{:.2f}'.format(free_space[bin_found.val])) - item_d)
            ...
            bins_tree.insert(new_key_f, bin_found.val)  # reinsert node with new capacity
        else:
            new_bin_id += 1
